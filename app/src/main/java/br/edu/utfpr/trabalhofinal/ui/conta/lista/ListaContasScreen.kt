...
import br.edu.utfpr.trabalhofinal.utils.colorir
...
private fun List(
    ...
    LazyColumn(modifier = modifier) {
        items(contas) { conta ->
            val icon =  if (conta.data > LocalDate.now()) {
                            Icons.Filled.ThumbDownOffAlt
                        } else {
                            Icons.Filled.ThumbUp
                        }
            val color = when(conta.tipo) {
                TipoContaEnum.DESPESA -> Color(0xFFCF5355)
                else -> Color(0xFF00984E)
            }
            val value = if (conta.tipo == TipoContaEnum.DESPESA) {
                   conta.valor.negate()
                } else {
                    conta.valor
                }
...
private fun BottomBar(
    ...
    val projection = contas.calcularProjecao()
    ...
    Column(
        ...
        Totalizador(
            ...
            valor = saldo,
            textColor = saldo.colorir()
        )
        Totalizador(
            modifier = Modifier.padding(bottom = 20.dp),
            titulo = stringResource(R.string.previsao),
            valor = projection,
            textColor = projection.colorir()
        )
    }
}
